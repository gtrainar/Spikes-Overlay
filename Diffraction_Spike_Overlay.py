...
class SpikeWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        hbox.addWidget(ctrl_widget, 0, Qt.AlignmentFlag.AlignBottom)
        ctrl_widget.setFixedWidth(320)

        # Preview pane
        preview_widget = QWidget()
        preview_layout = QVBoxLayout(preview_widget)
        hbox.addWidget(preview_widget, 2)
        ...
        # Layout
        ctrl_layout.addRow("FWHM Min:", self.fwhm_min_lbl)
        ctrl_layout.addRow("FWHM Max:", self.fwhm_max_lbl)
        ctrl_layout.addRow("Spike Length:", self.length_lbl)
        ctrl_layout.addRow("Spike Thickness:", self.thickness_lbl)
        ctrl_layout.addRow("Number of Spikes:", self.spike_count_combo)
        ctrl_layout.addRow("Blur Sigma:", self.blur_lbl)
        ctrl_layout.addRow("Rotation Angle:", self.rotation_lbl)
        ctrl_layout.addRow("Pattern Shape:", self.pattern_combo)

        ctrl_layout.addRow("Spike Color:", color_row_widget)
        ctrl_layout.addRow("Detected Stars:", self.star_count_label)

        ctrl_layout.addRow(spacer)
        ctrl_layout.addRow(btn_row_layout)
    ...
    def save_spiked_image(self):
        ...
        try:
            ...
            # Create a transparent image (all zeros)
    ...
    def show_original_image(self):
        """Show the original image without spikes."""
        if self.image_orig is None:
            return
    ...
    def show_help(self):
        """Display the help text in a scrollable dialog."""
        help_msg = (
            "Help File – Diffraction-Spike Generator\n"
            "\n1. Purpose\n"
            "This script adds synthetic diffraction spikes to a post-processed astronomical image.\n\n"
            "Why use it?\nTo simulate the appearance of telescope diffraction patterns for testing, visual effects, or pedagogical demonstrations.\n\n"
            "When to run it?\nAfter you have finished your normal image processing and want to add a realistic spike pattern before final export.\n\n"
            "2. Parameters (GUI Controls)\nControl\t\tDescription\n"
            "FWHM Min/Max\tFilter stars based on their average FWHM\n"
            "Spike Length\t\tLength of each spike in pixels (in '%' of FWHM)\n"
            "Spike Thickness\tWidth of each spike in pixels\n"
            "Number of Spikes\tHow many radial spikes to generate\n"
            "Blur Sigma\t\tGaussian blur factor\n"
            "Rotation Angle\tAngle in degrees (0–360) of the first spike relative to image X-axis\n"
            "Pattern Shape\tShape of the spikes\n"
            "Detected Stars\tDisplays a list of stars that will receive spikes (auto-updated)\n"
            "Preview Window\tShows the image with spikes in real time as you adjust parameters\n"
            "Save\t\tSaves a copy of the current image (with spikes) to the same directory\n"
            "Help\t\tOpens this help dialog\n\n"
            "Tip: The preview refreshes automatically after every parameter change, so you can fine-tune before saving.\n\n"
            "3. Workflow\n"
            "-Load Image – Open your processed FITS/PNG/JPEG image loaded in Siril\n"
            "-Set Parameters – Adjust the controls above; the preview updates instantly.\n"
            "-Review Detected Stars – Verify that the list under \"Detected Stars\" shows the stars you expect.\n"
            "-Preview – Use the preview window to confirm spike placement and appearance.\n"
            "-Save – Click Save to write a new file with spikes appended; the original remains untouched.\n"
            "-Help (Optional) – Click Help to view this quick-reference guide again.\n\n"
            "4. Notes\n"
            "The script works with common image formats (FITS, TIFF, PNG, JPEG).\n"
            "For best results, use images that have already been flat-fielded and bias-corrected.\n"
            "The spike generation algorithm is deterministic; identical settings produce the same output."
        )

        dlg = HelpDialog("Diffraction-Spike Generator – Help", help_msg, self)
        dlg.resize(700, 400)
        dlg.exec()
